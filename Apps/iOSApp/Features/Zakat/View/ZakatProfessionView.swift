...
public struct ZakatProfessionView: View {
    ...
    public var body: some View {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Text(appEnv.language.localizedString("zakat_desc_profession"))
                            .font(.caption)
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        VStack(alignment: .leading, spacing: 8) {
                            Text(appEnv.language.localizedString("zakat_label_salary"))
                            ...
                            TextField("0", text: $viewModel.currentAsset)
                                ...
                                .accessibilityLabel(appEnv.language.localizedString("zakat_label_salary"))
                        }
                        
                        VStack(alignment: .leading, spacing: 8) {
                            Text(appEnv.language.localizedString("zakat_label_other_income"))
                                .font(.subheadline.bold())
                                .foregroundColor(colors.foreground)
                            
                            TextField("0", text: $viewModel.secondaryIncome)
                                .padding()
                                .background(colors.foreground.opacity(0.04))
                                .cornerRadius(12)
                                .keyboardType(.decimalPad)
                                .accessibilityLabel(appEnv.language.localizedString("zakat_label_other_income"))
                        }
                        
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                Text(appEnv.language.localizedString("zakat_label_nisab_guide"))
                                    .font(.caption)
                        ...
                        Button(action: {
                            viewModel.selectedType = .profesi
                            viewModel.calculate()
                            viewModel.addRecord()
                        }) {
        ...
        .navigationTitle(appEnv.language.localizedString("zakat_title_profession"))
